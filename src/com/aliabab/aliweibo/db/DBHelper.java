...
import com.aliabab.aliweibo.db.DBInfo.DB;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // String
        // create_cus="CREATE TABLE IF NOT EXISTS Customer (_id integer primary key autoincrement, name varchar(20), age INTEGER)";
        db.execSQL(DBInfo.Table.USER_INFO_CREATE);
    }

    // onUpgrade()方法在数据库的版本发生变化时会被调用，一般在软件升级时才需改变版本号
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
         db.execSQL("DROP TABLE IF EXISTS "+DBInfo.Table.USER_INFO_TB_NAME);
         onCreate(db);
